...
//  Local MinMax fallback — only used when UWAPI is unreachable.
//  Do not remove: needed for offline development and testing.
...
/// MinMax (need to change to call UWAPI)
struct UWAPIGameAI {
    let game:GameState
    
    /// returns list of squares that contain pieces belonging to player
        ...
        for action in possibleActions() {
            ...
            if let winner = gameStatePostAction.currentWinner {
                ...
                if iterationCount < MAX_ITERATIONS {
                    let followUpActions = UWAPIGameAI(game: gameStatePostAction).scoredPossibleActions(playerBias: playerBias,
